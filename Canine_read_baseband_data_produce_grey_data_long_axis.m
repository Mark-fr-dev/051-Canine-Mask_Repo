...
elseif disease_name == BL_disease_name
    ...
    base_name ='rfDSEA16BL0';
    seq_id = 'rfDSEA16BL0';   % The folder name and the filename
end
...
for ele_slice = 8
    for ff = 16
      ...
      if ff == ff
       ...
       logdata = 20*log10(abs(PreImag(:,lat_slice,:))/MAX_RF);
       gray_data = (logdata+DR)/DR*255;
       gray_data = squeeze(gray_data);
...
%         image(PhiDegs,r(axi_s:end),gray_data)  
image(gray_data)  
        xlabel('ele (Degrees)')
        ylabel('Axial (mm)')
        colormap(gray(256))
        title(['Frame #' num2str(ff) 'Slice #' num2str(lat_slice)])
...
%             mask3 = abs(mask1 - mask2);
            ...
            mask = mask1(1:axial_range,1:end);
